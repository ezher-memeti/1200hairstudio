...
export default function BookingSection() {
  ...
  const selectedService =
    ...
    null;
  const selectedDate =
    dates.find((date) => date.id === state.dateId) ?? null;
  ...
  const panelContent = useMemo(() => {
    ...
    if (step === 0) {
      return <ServiceStep {...sharedProps} />;
    }

    if (step === 1) {
      return <DateTimeStep {...sharedProps} />;
    }
  ...
  }, [confirmed, selectedDate, selectedService, state, step]);
