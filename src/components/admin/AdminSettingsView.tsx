...
function Field({ label, value, onChange, type = "text", required = false, placeholder }: { label: string; value: string | number; onChange: (value: string) => void; type?: string; required?: boolean; placeholder?: string }) {
  return <label className="block"><span className={labelClass}>{label}</span><input type={type} required={required} value={value} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className={inputClass} /></label>;
}
...
export default function AdminSettingsView({ initialSettings, adminEmail, gmailConnected }: { initialSettings: AdminSettings; adminEmail: string; gmailConnected: boolean }) {
  ...
  const dirty = useMemo(() => ({ business: JSON.stringify(settings.business) !== JSON.stringify(saved.business), booking: JSON.stringify(settings.booking) !== JSON.stringify(saved.booking), notifications: JSON.stringify(settings.notifications) !== JSON.stringify(saved.notifications), finance: JSON.stringify(settings.finance) !== JSON.stringify(saved.finance), security: Boolean(password || confirmation) }), [settings, saved, password, confirmation]);
  ...
  function saveSection(section: Exclude<Section, "security">) {
    startTransition(async () => {
      const action = section === "business" ? saveBusinessSettings(settings.business) : section === "booking" ? saveBookingSettings(settings.booking) : section === "notifications" ? saveNotificationSettings(settings.notifications) : saveFinanceSettings(settings.finance);
      const result = await action;
      setNotice((current) => ({ ...current, [section]: { kind: result.success ? "success" : "error", text: result.message } }));
      if (result.success) setSaved((current) => ({ ...current, [section]: settings[section] }));
    });
  }
