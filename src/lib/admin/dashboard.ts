...
import { getCurrentRegisterDetail } from "@/lib/register/server";
...
export async function getAdminDashboardData() {
  ...
  const [todayAppointments, tomorrowAppointments, finance, services, register] = await Promise.all([
  ...
  return {today,tomorrow,now:now.toISOString(),todayAppointments,tomorrowAppointments,availableSlots,availabilityService:availabilityService??null,finance,todayComparison,monthComparison,sevenDayComparison,thirtyDayComparison,register};
}
